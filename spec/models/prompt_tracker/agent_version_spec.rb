  ...
  RSpec.describe AgentVersion, type: :model do
    ...
    describe "validations" do
      it "is valid with valid attributes" do
        version = AgentVersion.new(valid_attributes)
        expect(version).to be_valid
      end

      it "allows user_prompt to be optional" do
        version = AgentVersion.new(valid_attributes.except(:user_prompt))
        expect(version).to be_valid
      end

        it "requires system_prompt" do
          version = AgentVersion.new(valid_attributes.except(:system_prompt))
          expect(version).not_to be_valid
          expect(version.errors[:system_prompt]).to include("can't be blank")
        end

        it "auto-sets version_number if not provided" do
        version = AgentVersion.new(valid_attributes.except(:version_number))
        expect(version).to be_valid
        version.save!
      ...
      it "requires positive version_number" do
        version = AgentVersion.new(valid_attributes.merge(version_number: 0))
        expect(version).not_to be_valid

        version = AgentVersion.new(valid_attributes.merge(version_number: -1))
        expect(version).not_to be_valid
      end

      it "requires unique version_number per prompt" do
        AgentVersion.create!(valid_attributes)
        duplicate = AgentVersion.new(valid_attributes)
      ...
      it "allows same version_number for different prompts" do
        ...
        other_prompt = Agent.create!(name: "other_prompt")
        other_version = AgentVersion.new(valid_attributes.merge(agent: other_prompt))
        expect(other_version).to be_valid
      end

      it "requires valid status" do
        AgentVersion::STATUSES.each do |status|
          version = AgentVersion.new(valid_attributes.merge(status: status))
          expect(version).to be_valid, "Status '#{status}' should be valid"
        end

        version = AgentVersion.new(valid_attributes.merge(status: "invalid"))
        expect(version).not_to be_valid
      ...
      it "validates variables_schema is an array" do
        version = AgentVersion.new(valid_attributes.merge(variables_schema: []))
        expect(version).to be_valid

        # Use a prompt with no variables to prevent auto-extraction from overwriting
        version = AgentVersion.new(valid_attributes.merge(
      ...
      it "validates model_config is a hash" do
        version = AgentVersion.new(valid_attributes.merge(model_config: {}))
        expect(version).to be_valid

        version = AgentVersion.new(valid_attributes.merge(model_config: "not a hash"))
        expect(version).not_to be_valid
      ...
      describe "response_schema validation" do
        it "allows nil response_schema" do
          version = AgentVersion.new(valid_attributes.merge(response_schema: nil))
          expect(version).to be_valid
        end

        it "requires response_schema to be a hash" do
          version = AgentVersion.new(valid_attributes.merge(response_schema: "not a hash"))
          expect(version).not_to be_valid
        ...
        it "requires response_schema to have a type property" do
          version = AgentVersion.new(valid_attributes.merge(response_schema: { "properties" => {} }))
          expect(version).not_to be_valid
          expect(version.errors[:response_schema]).to include("must have a 'type' property")
        end

        it "requires object type to have properties" do
          version = AgentVersion.new(valid_attributes.merge(response_schema: { "type" => "object" }))
          expect(version).not_to be_valid
        ...
        it "accepts valid JSON Schema with object type" do
          ...
          version = AgentVersion.new(valid_attributes.merge(response_schema: schema))
          expect(version).to be_valid
        end
      ...
      describe "version immutability" do
        context "in development state (no tests, datasets, or responses)" do
          it "allows all changes" do
            version = AgentVersion.create!(valid_attributes)
        ...
        context "in testing state (has tests or datasets, no responses)" do
          let(:version) { AgentVersion.create!(valid_attributes) }
    ...
    describe "callbacks" do
      it "auto-increments version_number when not provided" do
        version1 = AgentVersion.create!(valid_attributes.except(:version_number))
        expect(version1.version_number).to eq(1)

        version2 = AgentVersion.create!(valid_attributes.except(:version_number))
        expect(version2.version_number).to eq(2)

        version3 = AgentVersion.create!(valid_attributes.except(:version_number))
    ...
    describe "scopes" do
      describe ".active" do
        it "returns only active versions" do
          active = AgentVersion.create!(valid_attributes.merge(status: "active"))
          deprecated = AgentVersion.create!(valid_attributes.merge(version_number: 2, status: "deprecated"))

          active_versions = AgentVersion.active
      ...
      describe ".deprecated" do
        it "returns only deprecated versions" do
          ...
          deprecated_versions = AgentVersion.deprecated
      ...
      describe ".draft" do
        it "returns only draft versions" do
          active = AgentVersion.create!(valid_attributes.merge(status: "active"))
          draft = AgentVersion.create!(valid_attributes.merge(version_number: 2, status: "draft"))

          draft_versions = AgentVersion.draft
      ...
      describe ".by_version" do
        it "orders by version_number descending" do
          v1 = AgentVersion.create!(valid_attributes.merge(version_number: 1))
          v3 = AgentVersion.create!(valid_attributes.merge(version_number: 3))
          v2 = AgentVersion.create!(valid_attributes.merge(version_number: 2))

          versions = prompt.agent_versions.by_version.to_a
    ...
    describe "#activate!" do
      it "sets status to active" do
        version = AgentVersion.create!(valid_attributes.merge(status: "draft"))
        version.activate!
        expect(version.reload.status).to eq("active")
      end

      it "deprecates other versions of same prompt" do
        v1 = AgentVersion.create!(valid_attributes.merge(version_number: 1, status: "active"))
        v2 = AgentVersion.create!(valid_attributes.merge(version_number: 2, status: "draft"))
      ...
      it "does not affect versions of other prompts" do
        other_prompt = Agent.create!(name: "other_prompt")
        other_version = AgentVersion.create!(valid_attributes.merge(agent: other_prompt, status: "active"))
    ...
    describe "#deprecate!" do
      it "sets status to deprecated" do
        version = AgentVersion.create!(valid_attributes.merge(status: "active"))
        version.deprecate!
    ...
    describe "status check methods" do
      ...
      it "#deprecated? returns true for deprecated versions" do
        version = AgentVersion.create!(valid_attributes.merge(status: "deprecated"))
    ...
    describe "#name" do
      it "returns formatted name without status for active versions" do
        version = AgentVersion.create!(valid_attributes.merge(version_number: 1, status: "active"))
        expect(version.name).to eq("v1")
      end

      it "returns formatted name with status for non-active versions" do
        version = AgentVersion.create!(valid_attributes.merge(version_number: 2, status: "draft"))
    ...
    describe "#structural_fields_changed?" do
      let(:version) do
        AgentVersion.create!(valid_attributes.merge(
    ...
    describe "#has_response_schema?" do
      ...
      it "returns true when response_schema is defined" do
        ...
        version = AgentVersion.create!(valid_attributes.merge(response_schema: schema))
    ...
    describe "#structured_output_enabled?" do
      ...
      it "returns false when model does not support structured output" do
        version = AgentVersion.create!(valid_attributes.merge(
    ...
    describe "archive functionality" do
      ...
      describe ".not_archived scope" do
        let!(:active_version) { AgentVersion.create!(valid_attributes) }
        let!(:archived_version) { AgentVersion.create!(valid_attributes.merge(version_number: 2)) }

        before { archived_version.archive! }

        it "returns only non-archived versions" do
          expect(AgentVersion.not_archived).to include(active_version)
          expect(AgentVersion.not_archived).not_to include(archived_version)
        end
      end
      ...
      describe ".archived scope" do
        ...
        it "returns only archived versions" do
          expect(AgentVersion.archived).to include(archived_version)
          expect(AgentVersion.archived).not_to include(active_version)
        end
      end
    end
  end
