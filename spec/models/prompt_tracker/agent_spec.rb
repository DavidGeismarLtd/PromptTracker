  ...
  RSpec.describe Agent, type: :model do
    ...
    describe "validations" do
      it "is valid with valid attributes" do
        prompt = Agent.new(valid_attributes)
        expect(prompt).to be_valid
      end

      it "requires name" do
        prompt = Agent.new(valid_attributes.except(:name))
        expect(prompt).not_to be_valid
        expect(prompt.errors[:name]).to include("can't be blank")
      end

      it "requires unique slug (case-insensitive)" do
        Agent.create!(valid_attributes.merge(slug: "test_slug"))
        duplicate = Agent.new(valid_attributes.merge(name: "Different Name", slug: "test_slug"))
      ...
      it "allows any name format but enforces slug format" do
        ...
        valid_names.each do |name|
          prompt = Agent.new(valid_attributes.merge(name: name, slug: "test_#{SecureRandom.hex(4)}"))
        ...
        valid_slugs.each do |slug|
          prompt = Agent.new(valid_attributes.merge(slug: slug))
    ...
    describe "associations" do
      let(:prompt) { Agent.create!(valid_attributes) }

      it "has many agent_versions" do
        expect(prompt).to respond_to(:agent_versions)
        expect(prompt.agent_versions.count).to eq(0)
      end

      it "destroys associated agent_versions when destroyed" do
        ...
        expect { prompt.destroy }.to change { AgentVersion.count }.by(-1)
      end
      ...
      it "has many llm_responses through agent_versions" do
    ...
    describe "scopes" do
      let!(:active_prompt) { Agent.create!(valid_attributes) }
      let!(:archived_prompt) { Agent.create!(valid_attributes.merge(name: "archived_prompt", archived_at: Time.current)) }

      describe ".active" do
        it "returns only non-archived prompts" do
          active_prompts = Agent.active
      ...
      describe ".archived" do
        it "returns only archived prompts" do
          archived_prompts = Agent.archived
    ...
    describe "instance methods" do
      ...
      describe "#active_version" do
        it "returns the active version" do
          active_version = prompt.agent_versions.create!(
              system_prompt: "You are a helpful assistant.",
            user_prompt: "Active version",
            version_number: 2,
            status: "active",
          )
          deprecated_version = prompt.agent_versions.create!(
              system_prompt: "You are a helpful assistant.",
      ...
      describe "#latest_version" do
        it "returns most recently created version" do
          first_version = prompt.agent_versions.create!(
              system_prompt: "You are a helpful assistant.",
            user_prompt: "First",
            version_number: 1,
            status: "deprecated",
          ...
          latest_version = prompt.agent_versions.create!(
              system_prompt: "You are a helpful assistant.",
            user_prompt: "Latest",
            version_number: 2,
            status: "active",
