...
module PromptTracker
  RSpec.describe TestsHelper, type: :helper do
    describe "#run_test_path" do
      context "when test belongs to a AgentVersion" do
      ...
      context "when test belongs to a AgentVersion with Assistants API" do
        let(:prompt) { create(:agent) }
        let(:version) { create(:agent_version, :with_assistants, agent: prompt) }
        let(:test) { create(:test, testable: version) }
    ...
    describe "#datasets_path_for_testable" do
      context "when testable is a AgentVersion" do
        let(:prompt) { create(:agent) }
        let(:version) { create(:agent_version, agent: prompt) }
      ...
      context "when testable is a AgentVersion with Assistants API" do
        let(:prompt) { create(:agent) }
        let(:version) { create(:agent_version, :with_assistants, agent: prompt) }
    ...
    describe "#load_more_runs_path_for_test" do
      context "when test belongs to a AgentVersion" do
        ...
        it "returns the correct path with pagination params" do
          expected_path = PromptTracker::Engine.routes.url_helpers
            .load_more_runs_testing_agent_version_test_path(version, test, offset: 10, limit: 5)
      ...
      context "when test belongs to a AgentVersion with Assistants API" do
        ...
        it "returns the correct path with pagination params" do
          expected_path = PromptTracker::Engine.routes.url_helpers
            .load_more_runs_testing_agent_version_test_path(version, test, offset: 0, limit: 10)
