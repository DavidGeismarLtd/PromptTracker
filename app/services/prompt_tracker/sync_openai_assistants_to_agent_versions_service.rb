...
module PromptTracker
  # Service to sync OpenAI Assistants to Prompts and AgentVersions.
  #
  # Fetches assistants from OpenAI API and creates:
  # - One Prompt per assistant (with slug based on assistant_id)
  # - One AgentVersion per prompt (version_number: 1)
  # - model_config[:api] = "assistants"
  # - model_config[:assistant_id] = "asst_abc123" (NEW field)
  # - model_config[:model] = "gpt-4o" (actual model name)
  #
  # This creates a 1:1:1 relationship: Assistant → Prompt → AgentVersion
  #
  # Delegates to RemoteEntity::Openai::Assistants::CreateAsAgentVersionService
  # for the actual creation of each Prompt and AgentVersion.
  #
  ...
  #   result[:created_versions]  # => [AgentVersion, ...]
  #
  class SyncOpenaiAssistantsToAgentVersionsService
    ...
    # Create a Prompt and AgentVersion from assistant data
    #
    # Delegates to CreateAsAgentVersionService for the actual creation.
    #
    ...
    def create_prompt_and_version_from_assistant(assistant_data)
      ...
      result = RemoteEntity::Openai::Assistants::CreateAsAgentVersionService.call(
        assistant_data: assistant_data
      )

      if result.success?
        @created_prompts << result.prompt
        @created_versions << result.agent_version
      else
