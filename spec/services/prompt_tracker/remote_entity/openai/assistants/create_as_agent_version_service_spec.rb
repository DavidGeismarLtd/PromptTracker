...
module PromptTracker
  module RemoteEntity
    module Openai
      module Assistants
        RSpec.describe CreateAsAgentVersionService do
          describe ".call" do
            ...
            it "creates a AgentVersion with correct attributes" do
              result = described_class.call(assistant_data: assistant_data)

              expect(result.agent_version).to be_persisted
              expect(result.agent_version.system_prompt).to eq("You are a helpful assistant.")
              expect(result.agent_version.version_number).to eq(1)
              expect(result.agent_version.status).to eq("draft")
            end

            it "sets model_config with assistant_id" do
              result = described_class.call(assistant_data: assistant_data)

              model_config = result.agent_version.model_config
            ...
            it "sets default user_prompt template" do
              ...
              expect(result.agent_version.user_prompt).to eq("{{user_message}}")
            end
            ...
            context "when slug already exists" do
              before do
                create(:agent, slug: "assistant_asst_abc123")
              end
            ...
            context "with tools in assistant data" do
              ...
              it "converts tools to PromptTracker format" do
                ...
                tools = result.agent_version.model_config["tools"]
            ...
            context "when creation fails" do
              before do
                allow(Agent).to receive(:create!).and_raise(ActiveRecord::RecordInvalid.new(Agent.new))
              end
              ...
              it "returns failure result" do
                ...
                expect(result.agent_version).to be_nil
